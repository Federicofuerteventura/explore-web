...
const exp = getExperienceBySlug('paddle-excursion')!

export const metadata: Metadata = {
  title: 'Paddle Excursion Fuerteventura | Guided Coastal Paddle',
  description:
    'Guided paddle excursion along the wild coastline of Fuerteventura. All levels welcome, small groups, all equipment included. Book directly by WhatsApp.',
  alternates: { canonical: `${SITE_URL}/experiences/paddle-excursion` },
  openGraph: {
    title: 'Paddle Excursion Fuerteventura — Explore the Coast from the Water',
    description:
      'Guided paddle excursion in Fuerteventura. Volcanic coastline, crystal water, max 8 per group. Book by WhatsApp.',
...
const schema = {
  ...
  url: `${SITE_URL}/experiences/paddle-excursion`,
  image: exp.image,
  offers: {
    '@type': 'Offer',
    price: exp.priceFrom,
  ...
  '@id': `${SITE_URL}/experiences/paddle-excursion`,
}
...
export default function PaddleExcursionPage() {
  return (
    <>
        ...
        <div className="container-main">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 lg:gap-14">
                ...
                <ul className="space-y-2.5" role="list">
                  {exp.includes.map(item => (
                    <li key={item} className="flex items-start gap-3 text-[var(--color-text-muted)]">
                      <span className="mt-0.5 w-5 h-5 rounded-full bg-ocean-100 text-ocean-600 flex items-center justify-center shrink-0">
                ...
                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2.5" role="list">
                  {exp.highlights.map(h => (
                    <li key={h} className="flex items-center gap-2.5 text-sm text-[var(--color-text-muted)]">
                      <span className="text-ocean-500" aria-hidden="true">✦</span> {h}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <aside className="lg:sticky lg:top-24 self-start space-y-4">
              ...
              {/* Other experience teaser */}
              <div className="card p-5 bg-amber-50 border-amber-100">
                <p className="text-xs font-semibold text-amber-700 uppercase tracking-wide mb-1">Also available</p>
                <p className="font-heading font-bold text-base text-[var(--color-text)] mb-2">West Sunset Experience 🌅</p>
                <Link
